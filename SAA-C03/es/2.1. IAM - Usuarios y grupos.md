...
1. Las políticas basadas en identidad (*Identity Based*) se pueden aplicar a identidades IAM:
   ...
   - **Administrada (*Managed*)**: Son aquellas que se crean con caracter "general" y reutilizables, es decir, son identificables (tienen su propio ARN) y se pueden aplicar a varias identidades. Existen 2 tipos de políticas administradas:
      - Administradas por AWS: Estas son políticas que crea y gestiona AWS. Se pueden identificar por símbolo de un cubo naranja.
      - Administradas por el cliente: Estas son las políticas que creamos nosotros manualmente. Una buena práctica es usar una política gestionada por AWS como plantilla y, si lo requerimos, realizar sobre esta los cambios que consideremos necesarios.
   ...
   - **Insertada (*Inline*)**: Son aquellas que se vinculan a una (1) identidad concreta de IAM y es creada para cumplir un caso muy específico/concreto. Dada su vinculación 1:1, no son reutilizables, no tienen identificador y si se elimina la identidad, la política también es eliminada con ella.
