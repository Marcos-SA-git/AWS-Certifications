...
- Haber completado el **Nivel 0** en la misma región (ej.: **eu-west-1**).
...
4. **Subnet name**: `subnet-priv-a` — **AZ**: `eu-west-1a` — **CIDR**: `10.1.1.0/24` → **Create subnet**.
...
> CloudShell por defecto. Comandos con `--region eu-west-1` **explícito** en todos los comandos. Copia los IDs manualmente (N0–N4 sin funciones ni variables avanzadas).
...
# 2) Borrar peering
aws ec2 delete-vpc-peering-connection --vpc-peering-connection-id <PCX_ID> --region eu-west-1

# 3) Instancia privada
aws ec2 terminate-instances --instance-ids <INSTANCE_PRIV_ID> --region eu-west-1

# 4) SG privada
aws ec2 delete-security-group --group-id <SG_PRIV_ID> --region eu-west-1

# 5) RT privada (desasociar si procede)
aws ec2 disassociate-route-table --association-id <RT_PRIV_ASSOC_ID> --region eu-west-1
aws ec2 delete-route-table --route-table-id <RT_PRIV_ID> --region eu-west-1

# 6) Subnet y VPC privada
aws ec2 delete-subnet --subnet-id <SUBNET_PRIV_ID> --region eu-west-1
aws ec2 delete-vpc --vpc-id <VPC_PRIV_ID> --region eu-west-1
```
