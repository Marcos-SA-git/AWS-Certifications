...
- Estado final del **Nivel 3** en la misma región (ej.: **eu-west-1**).
...
> CloudShell por defecto. Incluye `--region eu-west-1` **explícito** en todos los comandos. Copia los **IDs** manualmente (`<PCX_ID>`, `<RT_PRIV_ID>`, `<RT_SVC_PUB_ID>`, etc.). Sin funciones ni pipes avanzados.
...
aws ec2 delete-vpc-peering-connection --vpc-peering-connection-id <PCX_PRI_SVC_ID> --region eu-west-1
```
