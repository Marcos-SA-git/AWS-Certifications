...
- Región de trabajo coherente (ej.: **eu-west-1**).
...
> CloudShell por defecto. Incluye `--region eu-west-1` **explícito** en todos los comandos. Copia los **IDs** manualmente cuando se indiquen.
...
aws s3 cp objeto.txt s3://<BUCKET_UNICO>/objeto.txt --region eu-west-1
```
...
# aws ec2 delete-route --route-table-id <RT_PRIV_ID> --destination-cidr-block 0.0.0.0/0 --region eu-west-1
# Ahora:
curl -I http://s3.eu-west-1.amazonaws.com   # debe responder (403)
curl -I http://example.com                   # debe fallar
# Restaura la ruta por defecto al NAT si la quitaste:
# aws ec2 create-route --route-table-id <RT_PRIV_ID> --destination-cidr-block 0.0.0.0/0 --nat-gateway-id <NATGW_ID> --region eu-west-1
```
...
- Comprueba el **estado** del endpoint:
  ...
  ```bash
  aws ec2 describe-vpc-endpoints --vpc-endpoint-ids <GWEP_ID> --region eu-west-1
  ```
...
aws s3 rm s3://<BUCKET_UNICO>/objeto.txt --region eu-west-1
aws s3api delete-bucket --bucket <BUCKET_UNICO> --region eu-west-1
