...
- **Qué hace:** crea la Subnet `10.0.1.0/24` en la AZ `eu-east-1a` dentro de tu VPC y la etiqueta con **Name=subnet-pub-a**.
...
```bash
aws ec2 describe-instances --filters "Name=tag:Name,Values=EC2-WebPublica" --region eu-east-1
