...
- Un máximo de 50 peerings por VPC
- No es transitivo haciendo que la gestión de las tablas de enrutamiento fuera caótica y sujeta a errores humano.
...
Si VPC peering permitía la comunicación entre dos redes VPC, AWS PrivateLink permite la comunicación únicamente entre una ENI (*Elastic Network Interface*) en cada VPC. Por ponerlo en otras palabras, el emparejamiento entre VPCs es a nivel de red, mientras que PrivateLink es a nivel de aplicación o servicio.

Esta característica permite la comunicación directa y segura a un servicio sin necesidad de exponer toda la red como los emparejamientos, ni usando costosas y complejas VPNs, o haciendo que el tráfico tuviera que salir a internet exponiendo los datos o creando posibles brechas de seguridad.

A continuación, en el Esquema 5, podemos ver el flujo y funcionamiento de este servicio y los componentes necesarios:

```mermaid
---
title: "[Esquema 5] Flujo interno de AWS PrivateLink"
---
flowchart LR
    subgraph Consumer["VPC del Consumidor"]
        App["Aplicación Cliente"]
        IE["Interface Endpoint (ENI privada)"]
        App -->|"DNS privado resuelve a IP de ENI"| IE
    end

    subgraph AWS_Backbone["Red privada de AWS (infraestructura interna)"]
        Dummy1[" "]
        style Dummy1 fill:none,stroke:none
        PL["PrivateLink (Conexión Privada)"]
    end

    subgraph Provider["VPC del Proveedor"]
        NLB["Network Load Balancer"]
        EC2["Servicio Backend (EC2 / ECS)"]
        NLB --> EC2
    end
...
Como vemos en el esquema, gracias al Network Load Balancer (NLB) del lado del proveedor (*Provider*), el servicio que oferta queda "accesible" de forma privada dentro de la red de AWS, sin acceso público.

Luego, del lado del consumidor (*Consumer*) se configura el VPC Endpoint que no es más que un ENI que ocupará una dirección IP de la red del consumidor y que estará conectada con PrivateLink y, a su vez, vinculada con el servicio del proveedor.

Al establecer esta comunicación, el DNS del consumidor se configura automáticamente para resolver internamente las peticiones de la instancia o servicio hacia la ENI con PrivateLink, guiando el tráfico a través de AWS hasta el NLB, realizando el intercambio de forma privada y segura de extremo a extremo.

Hay que tener en cuenta que PrivateLink es unidireccional y funciona a nivel de capa de red usando TCP (Consumer -> Provider).

> 🔹**Ejemplos**:
>
> Si VPC peering es como conectar dos edificios enteros por un túnel, el PrivateLink es como tener una puerta mágica que solo conectase un despacho entre edificios.
>
> AWS PrivateLink funciona como un pasaje secreto bajo tierra. En lugar de salir a la vía pública (Internet) o pasar por estaciones visibles (NAT, VPN, Peering), el tráfico viaja por un túnel privado y directo dentro de la red de AWS, conectando solo tu aplicación con el servicio del proveedor, sin que nadie más sepa que el túnel existe.

### Gateway Endpoints

Los Gateway Endpoints permiten conectar tu VPC de manera privada y directa con servicios específicos de AWS (actualmente Amazon S3 y DynamoDB) sin usar Internet Gateway, NAT Gateway ni VPN.

En lugar de enrutar el tráfico hacia Internet (como ocurre por defecto), el Endpoint crea una ruta interna gestionada por AWS, asegurando que los datos nunca salgan de la red de Amazon.

A diferencia de PrivateLink (Interface Endpoints), los Gateway Endpoints no utilizan ENIs (Elastic Network Interfaces) ni direcciones IP dentro de subredes, sino que se integran directamente en la tabla de rutas (Route Table) de tu VPC.

Son gratuitos, altamente disponibles y seguros por diseño, ya que eliminan la necesidad de exponer tráfico a Internet o depender de servicios con costo adicional como los NAT Gateways.

A continuación, en el Esquema 6, puedes ver mejor cómo fluiría el tráfico según el destino que se requiriese.

```mermaid
---
title: [Esquema 6] AWS Gateway Endpoint
---
