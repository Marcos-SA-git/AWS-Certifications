...
Una **Nube Privada Virtual (*Virtual Private Cloud* o *VPC*)** podríamos orientarlo como tener nuestro propio centro de procesamiento de datos en la nube, con nuestros propios, servidores, aplicaciones, redes, ordenadores, almacenamiento, seguridad, etc. todo de manera virtual e interconectado como nosotros queramos.
...
- Las VPCs se organizan por región (refiriéndose a las diferentes regiones de AWS en el mundo que podemos seleccionar) y están limitadas a 5 por región, por cuenta (salvo raras excepciones).
...
- Se puede crear y asociar un set de opciones personalizado con una VPC o no usar DHCP directamente
...
- El emparejamiento no permite el tráfico transitivo (por defecto), es decir, debe haber emparejamiento **directo** entre VPCs para poder hacer llegar el tráfico hasta otro VPC por más que haya otras VPCs que podrian hacer de intermediarias. (Véase el Esquema 2)
...
flowchart LR
  ...
  VPC_A <-. 🚫**NO transitivo**🚫<br>El tráfico no puede pasar por VPC-B .-> VPC_C
```
...
Dentro de AWS existen las **Puertas de enlace NAT (*NAT Gateways*)**, permitiendo a las instancias de una subred privada puedan comunicarse con redes externar, pero sin que servicios externos inicialicen una conexión con dichas instancias. Esto permite:
...
- Escalabilidad: Es automaticamente escalable desde 5Gbps hasta 100Gbps.
...
```mermaid
  ---
  title: "NAT Gateway y NACLs — Control del tráfico en subredes privadas"
  ---
  flowchart LR
      subgraph "🌍"
          I[Internet]
      ...
      subgraph Public_Subnet["Public Subnet con NAT Gateway"]
          NGW[NAT Gateway<br/>❌ sin Security Group]
          IGW[Internet Gateway]
      end

      subgraph Private_Subnet["Private Subnet con EC2"]
          EC2[EC2 Instance<br/>Security Group ✅]
      end

      EC2 -->|"Tráfico saliente<br>(HTTP/HTTPS)"| NGW
      NGW -->|"Traduce IP privada -> IP pública (Elastic IP)"| IGW
      IGW --> I

      I -->|Respuesta si se inició desde dentro| IGW
      IGW --> NGW
      NGW --> EC2

      NACLNote[Nota NACL: controla el tráfico a nivel de Subnet]
      NGWNote[Nota: no se pueden aplicar Security Groups al NAT Gateway]

      Public_Subnet -.-> NACLNote
      NGW -.-> NGWNote

      classDef note fill:#ffffff,stroke:#888,stroke-dasharray: 5 5,color:#333,font-size:12px;
      class NACLNote,NGWNote note
```
