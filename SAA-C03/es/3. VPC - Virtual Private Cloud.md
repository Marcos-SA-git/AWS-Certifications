...
### 4.1 Creación de una VPC para producción

Diseñar una **VPC personalizada** con CIDR propio (por ejemplo, `10.0.0.0/16`) para aislar entornos de desarrollo, pruebas y producción.
Cada entorno tiene su **propia VPC**, evitando solapamiento de direcciones y facilitando control de acceso y facturación.
Usar **IPv6 opcional** (`/56` para la VPC, `/64` para subredes) y prever el crecimiento a futuro.

### 4.2 Segmentación mediante subredes públicas y privadas

Dividir la VPC en subredes según su función:

- **Subred pública**: contiene balanceadores de carga (ALB/NLB), bastiones o NAT Gateways.
- **Subred privada**: aloja instancias EC2, bases de datos y servicios internos.
La **tabla de rutas** de las subredes públicas apunta a un **Internet Gateway**, mientras que las privadas usan **NAT Gateway** o **VPC Endpoints** para el tráfico saliente.

### 4.3 Control de tráfico con NACLs y Security Groups

Configurar **NACLs** para definir los límites de red por subred (reglas de entrada y salida explícitas, ordenadas).
Complementarlas con **Security Groups**, que protegen a nivel de recurso (ENI/instancia).  
Ejemplo:

- NACL permite puertos 80, 443 y el rango de puertos efímeros.
- SG del servidor web permite HTTP/HTTPS desde cualquier IP y SSH solo desde la IP del administrador.

### 4.4 Uso de puertos efímeros en tráfico saliente

Permitir en las NACLs el rango **1024–65535** para el tráfico de retorno.
Esto evita bloqueos cuando las instancias inician conexiones hacia fuera (actualizaciones, repositorios, etc.).  
Es un caso típico en subredes privadas con NAT Gateway.

### 4.5 Implementación de NAT Gateway para subredes privadas

Cuando los recursos privados necesitan acceso a Internet (por ejemplo, para descargar parches o dependencias), se usa un **NAT Gateway** desplegado en una subred pública.
El tráfico sale con una IP pública estática (**Elastic IP**) y las respuestas regresan al origen interno.
Para alta disponibilidad, se recomienda un NAT por **zona de disponibilidad (AZ)**.

### 4.6 Comparación entre NAT Instance y NAT Gateway

Históricamente se usaban **NAT Instances (EC2)** configuradas manualmente.
Hoy se reemplazan por **NAT Gateways**, que son **administrados, escalables y sin mantenimiento**.
El examen puede mencionarlas como legado: recordad que requieren desactivar el *source/dest check*.

### 4.7 Comunicación directa entre VPCs mediante Peering

Permite conectar dos VPCs dentro de una misma región o entre regiones diferentes.
Ideal para compartir recursos o datos internos sin exponerlos públicamente.
Requisitos clave:

- CIDR no solapados.
- Rutas configuradas en ambas VPCs.
- No hay **tráfico transitivo** (A–B y B–C no implica A–C).

### 4.8 Escalado de conectividad con Transit VPC / Transit Gateway

Cuando el número de VPCs crece, usar un **punto central de enrutamiento** (Transit VPC o **AWS Transit Gateway**) simplifica la administración.
Todas las VPCs se conectan al hub y el tráfico fluye internamente sin conexiones punto a punto.
El **Transit Gateway** es la versión moderna y administrada por AWS.

### 4.9 Acceso privado a servicios AWS con Gateway Endpoints

En subredes privadas, crear **Gateway Endpoints** para **S3** o **DynamoDB**, evitando el uso de NAT o Internet Gateway.  
El tráfico permanece dentro del backbone de AWS.  
En la Route Table, se agrega una entrada tipo `com.amazonaws.<region>.s3 → vpce-xxxxxx`.  
Sin coste adicional y con alta disponibilidad.

### 4.10 Acceso privado a APIs y servicios gestionados con Interface Endpoints

Para servicios como **SSM, ECR, KMS, CloudWatch o Secrets Manager**, usar **Interface Endpoints**.
Estos crean una **ENI privada** con su propio Security Group dentro de la subred.
El DNS interno resuelve los nombres de AWS a la IP privada del endpoint.
Ideal para entornos cerrados sin salida a Internet.

### 4.11 Comunicación entre VPCs y cuentas mediante AWS PrivateLink

Cuando necesitas que **otra cuenta o VPC consuma un servicio específico** (no toda la red), se usa **PrivateLink**.  
El **proveedor (Provider)** publica un servicio a través de un **Network Load Balancer (NLB)**.
El **consumidor (Consumer)** lo accede mediante un **Interface Endpoint** privado.
El tráfico fluye **unidireccionalmente (Consumer → Provider)** dentro de la red de AWS.
No requiere VPC Peering, IGW ni NAT.

### 4.12 Administración segura con Bastion Host o Session Manager

Para acceder a instancias en subredes privadas:

- Usar un **Bastion Host** en subred pública (con IP pública y SG restringido).
- Alternativamente, **AWS Systems Manager Session Manager** evita abrir puertos SSH y no requiere bastion.
Ambos métodos permiten administración segura sin exponer la infraestructura.

### 4.13 Personalización de DHCP Option Sets

Definir un **DHCP Option Set** personalizado con:

- Dominio interno (por ejemplo, `corp.local`).
- Servidor DNS (`AmazonProvidedDNS` o uno propio).
- Servidor NTP.
Útil en entornos corporativos o híbridos para coherencia de nombres y sincronización.

### 4.14 Diseño multi-cuenta con VPC Endpoints compartidos

En arquitecturas de tipo *Landing Zone*:

- Cada cuenta (network, shared-services, prod, dev) tiene su VPC aislada.
- Los servicios compartidos (por ejemplo, CI/CD, logging, seguridad) se exponen vía **PrivateLink**.
- El resto de cuentas accede a ellos mediante **Interface Endpoints**, manteniendo tráfico 100% privado y centralizado.

### 4.15 Optimización de costos y resiliencia en conectividad

- Reducir egreso por NAT usando **Gateway Endpoints**.
- Implementar **Interface Endpoints** solo para los servicios más usados.
- Usar **NAT por AZ** para resiliencia o **compartido** si el presupuesto es limitado.
- Monitorizar **VPC Flow Logs** para ajustar rutas y detectar tráfico inesperado.
